package com.example.yf.creatorshirt.mvp.ui.view.freshrecyler;
...
public class FreshRecyclerView extends RecyclerView {
    ...
    public FreshRecyclerView(Context context) {
        this(context, null);
    }

    public FreshRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FreshRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        if (pullRefreshEnabled) {
            RecyRefreshHeader refreshHeader = new RecyRefreshHeader(context);
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof RecyRefreshHeader)) {
            RecyRefreshHeader refreshHeader = new RecyRefreshHeader(getContext());
            mHeaderViews.put(0, refreshHeader);
            mRefreshHeader = refreshHeader;
        }
    ...
    public void refreshComplete() {
        ...
        Log.i("newsFragment", " refreshComplete");
        if (isLoadingData) {
            loadMoreComplete();
        } else {
            mRefreshHeader.refreshComplate();
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    && lastVisibleItemPosition >= layoutManager.getItemCount() - 1 && layoutManager.getItemCount() > layoutManager.getChildCount() && !isnomore && mRefreshHeader.getState() < RecyRefreshHeader.STATE_REFRESHING) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisiableHeight() > 0 && mRefreshHeader.getState() < RecyRefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
